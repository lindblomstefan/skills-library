...
5. Recommend only after a repo read or concrete typed answers exist. Read `~/.claude/skills/skills-library/references/catalog-overview.md` and use only the Library Skills section as candidates. For each candidate, also read `~/.claude/skills/skills-library/feedback/<skill-id>.md` if it exists. Return at most 5 skills: license state, validation status, blockers, and standard-ready vs exploratory. Do not add any skill not in the Library Skills section — no built-in skills, no locally installed skills, no exceptions.
